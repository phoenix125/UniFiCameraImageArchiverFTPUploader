...
Local $ai_FTPReadFile = DllCall($__g_hWinInet_FTP, 'bool', 'InternetReadFile', 'handle', $hFTPFile, 'struct*', $tBuffer, 'dword', $iNumberOfBytesToRead, 'dword*', 0)
...
$s_FileRead = BinaryMid(DllStructGetData($tBuffer, 1), 1, $iNumberOfBytesRead)
Else
$s_FileRead = DllStructGetData($tBuffer, 1)
EndIf
...
EndFunc
Func _ImageResizer($tFileBefore, $tFileAfter, $tWidth = 16, $tHeight = 16)
_GDIPlus_Startup()
...
LogWrite("[Update] !!! New " & $aUtilName & " update available. Installed version: " & $tUtil & ", Latest version: " & $tVer[0], "[Update] New " & $aUtilName & " update available. Installed version: " & $tUtil & ", Latest version: " & $tVer[0] & ", Notes: " & $tTxt1)
...
Local $tMB = 6
Else
Local $tMB = MsgBox($MB_YESNOCANCEL, $aUtilName, "New " & $aUtilName & " update available. " & @CRLF & "Installed version: " & $tUtil & @CRLF & "Latest version: " & $tVer[0] & @CRLF & @CRLF & "Notes: " & @CRLF & $tVer[1] & @CRLF & @CRLF & "Click (YES) to download update to " & @CRLF & @ScriptDir & @CRLF & "Click (NO) to stop checking for updates." & @CRLF & "Click (CANCEL) to skip this update check.", 15)
EndIf
If $tMB = 6 Then
...
Local $gBXstart = 58, $gBYstart = 43, $gBGapX = 1, $gBGapY = 1, $gBinaRow = 25, $gBW = 27, $gBH = 25, $gY = -20
For $i = 0 To ($aNumberOfEntries - 1)
If Mod($i + 1, $gBinaRow) = 0 And $i < ($aNumberOfEntries - 1) Then $gY += $gBH + $gBGapY
Next
Global $tY = $gY
...
EndFunc
Func F_I_URLChange()
Local $tTxt = GUICtrlRead($F_I_URL)
